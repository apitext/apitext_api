<?php
/**
 * TEIParser allows a user to extract text content from a XML String based upon...
 */
class TEIParser{
	...
	private $tagList; // A structure for storing a list of tags
	
	/**
	 * Initializes a new TEIParser using the provided XML Parser callback
	 * function names.
	 */
	public function TEIParser($openTag, $characterData, $closeTag, $tagMap) {
	  $this->parser = NULL;
	  $this->data = "";
	...
	/**
	 * A callback function for XML Parser which adds a single space to this
	 * TEI parser's collected text data.
	 */
	private function textOpenTag($parser, $name, $attrs) {
		$this->data .= " ";
	}
	 ...
	 * TEI parser's collected text data.
	 */
	private function textCharacterData($parser, $data) {
		$this->data .= trim($data);
	}
	 ...
	 * TEI parser's collected text data.
	 */	
	private function textCloseTag($parser, $name) {
		$this->data .= " ";
	}
	 ...
	 * 
	 */
	private function annotationOpenTag($parser, $name, $attrs) {
		array_push($this->tagList, $name);
	}

	/**
	 * 
	 */
	private function annotationCharacterData($parser, $name) {}
	
	/**
	 * 
	 */
	private function annotationCloseTag($parser, $name) {}
	...
	/**
	 * Returns this TEI Parsers collected tag list.
	 */	
